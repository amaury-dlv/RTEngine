...
static void     hashCoord(double HC[6], int *X, int *Y, int *Z);
...
double pnoise(double x, double y, double z)
{
  ...
  w = fade(z);
  hashCoord(HC, &X, &Y, &Z);
  return (lerp(w, lerp(v, lerp(u, grad(p[(int)HC[1]], x, y, z),
   grad(p[(int)HC[4]], x - 1, y, z)), lerp(u, grad(p[(int)HC[2]], x, y - 1, z),
   grad(p[(int)HC[5]], x - 1, y - 1, z))),
   lerp(v, lerp(u, grad(p[(int)HC[1] + 1], x, y, z - 1),
   grad(p[(int)HC[4] + 1], x - 1, y, z - 1)),
   lerp(u, grad(p[(int)HC[2] + 1], x, y - 1, z - 1),
   grad(p[(int)HC[5] + 1], x - 1, y - 1, z - 1)))));
}
